...
class BlackjackMDP(util.MDP):
    ...
    def succAndProbReward(self, state, action):
        ...
        cardValSum = state[0] #initial is 0 since the player has no card
        ...
        # if the action is Take
            ...
            if state[1] != None:
                #add known card val to hand val
                cardValSum += self.cardValues[state[1]]
                # check if the game is busted
                if cardValSum <=  self.threshold: #not busted
                    deck = list(state[2])
                    deck[state[1]] -= 1
                    deck = tuple(deck)
                    # deck empty?
                    if sum(deck) == 0:
                        reward = cardValSum
                        newState = (cardValSum, None, None)
                    else:
                        reward = 0
                        newState = (cardValSum, None, deck)
                # if its busted, sum of face val is > than threshold
                else:
                    reward = 0
                    #set state[1] = None since it got busted
                    newState = (cardValSum, None, None)
            ...
            # if the last action is not peek
                ...
                for i in range(len(self.cardValues)):
                    if state[2][i] != 0:
                        reward = 0
                        totalCards += 1
                        #add unknown card val to hand val
                        cardValSum += self.cardValues[i]
                        # if not busted 
                        if cardValSum <= self.threshold:
                            deck = list(state[2])
                            deck[i] -= 1
                            deck = tuple(deck)
                            # run out of cards
                            if sum(deck) == 0:
                                #get reward of hand value
                                reward = cardValSum
                                newState = (cardValSum, None, None)
                            else:
                                reward = 0
                                newState = (cardValSum, state[1], deck)
                        # if busted
                        else:
                            #get rewards of hand value = 0
                            reward = 0
                            newState = (cardValSum, None, None)
                        results.append(newState)
                        rewards.append(reward)
                        # reset the total card value since the game ends
                        cardValSum = state[0]
                temp = list()
                for index, val in enumerate(results):
                    temp.append( (val, 1/float(totalCards), rewards[index]))
        ...
        # if the action is Peek
            ...
            if state[1] != None:
                ...
                results = list()
                for i in range(len(self.cardValues)):
                    #add known card val to the hand val
                    if state[2][i] > 0:
                        results.append((cardValSum,i,state[2]))
            #get reward of negative peekCost
            temp = list()
            for val in results:
                temp.append( (val, 1/float(len(results)), -self.peekCost) )
            return temp
        ...
        # if the action is Quit
            ...
            #check if got busted
            if ( cardValSum <= self.threshold ): #not busted
                return [((cardValSum, None, None), 1, cardValSum)]
            else: #busted
                return [((cardValSum, None, None), 1, 0)]
...
def peekingMDP():
    ...
    """
    # BEGIN_YOUR_CODE (our solution is 1 line of code, but don't worry if you deviate from this)
    return BlackjackMDP(cardValues=[16,3,5], multiplicity=3, threshold=20, peekCost=1)
    # END_YOUR_CODE
