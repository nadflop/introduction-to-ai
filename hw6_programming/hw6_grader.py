...
def runq4():
  ...
  params = '-l contestClassic -p ExpectimaxAgent -a evalFn=better,depth=2 -q -n %d -c' % nGames
...
def testq4():
  ...
  if timeouts > 0:
    grader.fail('Agent timed out on contestClassic with betterEvaluationFunction. No autograder feedback will be provided.')
    firstTime = False
    return
  if wins == 0:
    ...
    firstTime = False
    return
  for score in range(2500, 3500, 100):
    if averageWinScore >= score: grader.addPoints(1)
  if firstTime:
    grader.setSide({'score': averageWinScore})
  firstTime = False

grader.addManualPart('4a', 5, extraCredit=True, description='Points for placing in the top 3')

grader.addBasicPart('4a-1-basic', lambda : testq4(), 5, maxSeconds=60, extraCredit=True, description='1 extra credit point per 100 point increase above 2400.')
